...
for( iStep=1; iStep<=maxStep; iStep++ ) {
    ...
    normV = sqrt( (v[1]+tau*k_1[1])*(v[1]+tau*k_1[1]) + (v[2]+tau*k_1[2])*(v[2]+tau*k_1[2]) );
    accel[1] = air_const*normV*(v[1]+tau*k_1[1]); // 空気抵抗
    accel[2] = air_const*normV*(v[2]+tau*k_1[2]); // 空気抵抗
    accel[2] -= grav; // 重力

    k_2[1] = accel[1];
    k_2[2] = accel[2];

    // rは、tの一次式なので、rの数値計算は一次のテイラー展開（オイラー法）で十分。
    r[1] = r[1] + tau*v[1];
    r[2] = r[2] + tau*v[2];

    v[1] = v[1] + 0.5*tau*k_1[1] + 0.5*tau*k_2[1];
    v[2] = v[2] + 0.5*tau*k_1[2] + 0.5*tau*k_2[2];
